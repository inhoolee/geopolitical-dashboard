...
| 3 | **ACLED (2017–present)** | Ensure regional snapshots exist under `data/raw/acled/*_aggregated_data_up_to-*.csv`, then run `uv run python scripts/run_pipeline.py --sources acled`. |
...
| ACLED | 0 | ⚠️ Needs local regional CSV snapshots in `data/raw/acled/` |
